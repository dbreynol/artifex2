...
Write your own program to plot the autocorrelation function of a given time series, for a specified number of lags.

The autocorrelation between a time series $y$ at lag $k$ is:
...
For your function, accept two arguments: the lag value and the time series. Output the acf for the given lag. Once you complete this, use that function in a new function to plot the first 5 $r_k$ values using barplots. Compare with <code>acf</code>.